...
export function Button({ text, onClick, type="filled", disable=false, icon=false }) {
  ...
  const boxSx = useMemo(() => {
    switch (type) {
      ...
      case "outlined":
        return {
          border: `1px solid ${disable ? theme.palette.neutral.main : theme.palette.neutral.midLight}`,
        };
      case "text":
        return disable ? {} : {
          '&:hover': { background: theme.palette.stateLayer.primary8 },
          '&:Mui-focusVisible': { background: theme.palette.stateLayer.primary12 },
        };
      default:
        return disable ? theme.palette.neutral.midLight : theme.palette.primary.main;
    }
  }, [type, disable, theme]);
    ...
    switch (type) {
      case "filled":
        return disable ? {
          ...
          '&.Mui-focusVisible': { background: theme.palette.stateLayer.onPrimary12 },
        };
      case "outlined":
        return disable ? {} : {
          '&:hover': { background: theme.palette.stateLayer.primary8 },
  ...
  const textColor = useMemo(() => {
    switch (type) {
      case "filled":
        return disable ? theme.palette.neutral.main : theme.palette.neutral.light;
      case "outlined":
        return disable ? theme.palette.neutral.main : theme.palette.primary.main;
      case "text":
        return disable ? theme.palette.neutral.main : theme.palette.primary.main;
      default:
        return disable ? theme.palette.neutral.main : theme.palette.neutral.light;
  ...
  return (
    ...
    >
      <Box sx={{
        ...
        <Typography variant="label" sx={{ color: textColor, textAlign: 'center' }} >
          {text}
        </Typography>
      </Box>
    </Stack>
